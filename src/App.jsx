...
import { motion } from 'framer-motion'; // Tích hợp thư viện hiệu ứng chuyên nghiệp
...
export default function App() {
  ...
  return (
    <div className="bg-[#030712] text-slate-300 min-h-screen font-sans selection:bg-cyan-500/30 relative overflow-x-hidden">
      ...
      <div className="fixed top-6 right-6 z-50 flex items-center bg-slate-950/80 backdrop-blur-md border border-slate-800/60 p-1 rounded-xl gap-1 shadow-xl">
        {['vi', 'en', 'ja'].map(l => (
          <button 
            key={l}
            onClick={() => setLang(l)} 
      ...
      {/* FIXED NAVIGATION */}
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 bg-slate-950/70 backdrop-blur-xl border border-slate-800/80 px-4 md:px-6 py-3 rounded-full z-45 flex items-center gap-4 md:gap-6 shadow-2xl max-w-[95%]">
      ...
      <section id="hero" className="max-w-6xl mx-auto px-6 pt-36 pb-20 min-h-[90vh] flex flex-col lg:flex-row items-center justify-between gap-12 target:scroll-mt-36">
        <div className="flex-1 space-y-6 text-left">
          ...
          <p className="text-xl text-slate-400 font-medium">Software Engineer & Industrial R&D Specialist</p>
          <p className="text-slate-400 max-w-xl leading-relaxed text-sm md:text-base">{t.heroDesc}</p>
          
          <div className="flex flex-wrap gap-3 pt-2">
            <a href="#experience" className="bg-linear-to-r from-cyan-500 to-blue-600 text-slate-950 font-bold px-5 py-3 rounded-xl shadow-lg hover:opacity-90 hover:scale-[1.01] transition-all flex items-center gap-2 text-xs cursor-pointer">
              {t.viewProjects}
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
            </a>
            <a href="https://github.com/nguyenbao1682002" target="_blank" rel="noreferrer" className="bg-slate-900 border border-slate-800 text-white font-medium px-4 py-3 rounded-xl hover:bg-slate-800 transition-all flex items-center gap-2 text-xs">
            ...
            <a href="https://linkedin.com/in/baonguyen2002tech" target="_blank" rel="noreferrer" className="bg-slate-900 border border-slate-800 text-white font-medium px-4 py-3 rounded-xl hover:bg-slate-800 transition-all flex items-center gap-2 text-xs">
        ...
        <div className="w-full lg:w-96 bg-slate-900/30 backdrop-blur-md border border-slate-800/80 p-6 rounded-2xl relative">
          ...
          <a 
            ...
            download 
            className="w-full block text-center bg-linear-to-r from-purple-500 to-pink-500 text-white font-bold py-3 px-4 rounded-xl text-xs hover:opacity-90 hover:scale-[1.01] transition-all cursor-pointer shadow-lg shadow-purple-500/20"
          >
            {t.downloadResume}
          </a>
        </div>
      ...
      <section id="about" className="max-w-4xl mx-auto px-6 py-20 border-t border-slate-900">
        <h2 className="text-xs font-mono tracking-widest text-cyan-400 uppercase mb-2">{t.aboutSub}</h2>
        <h3 className="text-3xl font-bold text-white mb-6">{t.aboutTitle}</h3>
      ...
      <section id="experience" className="max-w-6xl mx-auto px-6 py-20 border-t border-slate-900">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            <h2 className="text-xs font-mono tracking-widest text-purple-400 uppercase mb-2">{t.expSub}</h2>
            <h3 className="text-3xl font-bold text-white">{t.expTitle}</h3>
          </div>
          
          <div className="flex flex-wrap bg-slate-900/80 p-1 rounded-xl border border-slate-800 text-[11px] font-bold">
              ...
              <div className="space-y-6">
                <div className="grid gap-6">
                  {experience.projects.map((project, projIndex) => (
                    <div key={projIndex} className="bg-slate-900/20 backdrop-blur-md border border-slate-800/80 p-6 rounded-2xl hover:border-slate-700 transition-all flex flex-col md:flex-row gap-6 justify-between items-center group">
      ...
      <section id="contact" className="max-w-4xl mx-auto px-6 py-20 border-t border-slate-900 text-center">
        <h2 className="text-xs font-mono tracking-widest text-cyan-400 uppercase mb-2">{t.contactSub}</h2>
        <h3 className="text-3xl font-extrabold text-white mb-4">{t.contactTitle}</h3>
        <p className="text-slate-400 max-w-md mx-auto mb-10 text-xs md:text-sm leading-relaxed">{t.contactDesc}</p>

        <div className="max-w-xs mx-auto bg-slate-900/50 border border-slate-800 p-4 rounded-xl space-y-3 text-left text-xs mb-8">
        ...
        <a href={gmailComposeUrl} target="_blank" rel="noreferrer" className="inline-block bg-cyan-500 text-slate-950 font-bold px-6 py-3 rounded-xl hover:opacity-90 text-xs shadow-md cursor-pointer">
          Send Email
        </a>
      </section>
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl relative flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 flex flex-col items-center justify-center min-h-[300px] bg-slate-950/20 overflow-y-auto">
              {selectedProject.demo ? (
                    ...
                    {selectedProject.demo.images && selectedProject.demo.images.length > 0 ? (
                      <>
                        <img 
                          ...
                          className="w-full h-auto max-h-[50vh] object-contain rounded-xl border border-slate-800 shadow-xl transition-all duration-300"
                        />
                ...
                <div className="text-center py-6 space-y-4 max-w-md">
